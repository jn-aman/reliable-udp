...
def tests_to_run(forwarder):
    ...
    BasicTest.BasicTest(forwarder, "data.txt")
    RandomDropTest.RandomDropTest(forwarder, "data.txt")
...
class Forwarder(object):
    ...
    def start(self, input_file):
        ...
        receiver = subprocess.Popen(["python2", self.receiver_path,
                                     "-p", str(self.receiver_port)])
        time.sleep(0.2) # make sure the receiver is started first
        sender = subprocess.Popen(["python2", self.sender_path,
                                   "-f", input_file,
                                   "-p", str(self.port)])
        try:
...
if __name__ == "__main__":
    ...
    def usage():
        print "Forwarder/Test harness for ADHS"
